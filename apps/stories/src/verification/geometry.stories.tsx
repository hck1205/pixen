...
export const Matrix: Story = matrixStory(GEOMETRY_CLAIMS);
...
export const Spaces: Story = () => {
  const result = useAsync(() => createSampleImage({ width: 1200, height: 800 }).then(walk), []);
      ...
      {result && (
        <>
          <DataTable
            ...
            keyOf={(row) => row.step}
            columns={[
              { header: "Step", cell: (row) => row.step, style: { fontWeight: 600, whiteSpace: "nowrap" } },
              { header: "Image", cell: (row) => row.image },
              { header: "Stage", cell: (row) => row.stage },
              { header: "Output", cell: (row) => row.output },
            ]}
          />
          <p style={note}>
