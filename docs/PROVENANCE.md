...
| `video/cost.ts` | Saying what an export will cost before it starts | Ours, from our own measurement: 1.00× wall clock and about 0.2s per seek between kept parts. That a threshold is a moment to offer a server rather than a refusal is our own decision |
| `video/player.ts` | Playing the kept parts rather than the file | MDN/WHATWG: `HTMLMediaElement` — `play`, `pause`, `currentTime`, `muted`. That the playhead skips what was cut, that a position inside a cut reads as the start of what follows, and that the player reports what it was asked for rather than what the element is doing, are ours — the last because we measured an export making the element fire events a host never asked for |
| `model/annotations.ts` (`space`) | Which frame a layer belongs to | Ours. The requirement — some shapes should not turn with the picture — was taken from the supplied documentation; the model is our own, and deliberately reuses two of the four coordinate spaces this project already documents rather than inventing a parallel vocabulary. Measured through the real export: an output-space bar stayed in the corner across a quarter turn while an image-space one moved with the picture |
